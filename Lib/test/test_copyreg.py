...
class CopyRegTestCase(unittest.TestCase):
    ...
    def test_extension_registry(self):
        mod, func_, code = 'junk1 ', ' junk2', 0xabcd
        e = ExtensionSaver(code)
        try:
            # Shouldn't be in registry now.
            self.assertRaises(ValueError, copyreg.remove_extension,
                              mod, func_, code)
            copyreg.add_extension(mod, func_, code)
            # Should be in the registry.
            self.assertTrue(copyreg._extension_registry[mod, func_] == code)
            self.assertTrue(copyreg._inverted_registry[code] == (mod, func_))
            # Shouldn't be in the cache.
            self.assertNotIn(code, copyreg._extension_cache)
            # Redundant registration should be OK.
            copyreg.add_extension(mod, func_, code)  # shouldn't blow up
            # Conflicting code.
            ...
            if code + 1 not in copyreg._inverted_registry:
                self.assertRaises(ValueError, copyreg.remove_extension,
                                  mod[1:], func_[1:], code + 1)

        finally:
            e.restore()

        # Shouldn't be there anymore.
        self.assertNotIn((mod, func_), copyreg._extension_registry)
        ...
        for code in 1, 0x7fffffff:
            ...
            try:
                copyreg.add_extension(mod, func_, code)
                copyreg.remove_extension(mod, func_, code)
            finally:
                e.restore()
