    ...
    def test_function_tp_clear_leaves_consistent_state(self):
        ...
        class LateFin:
            ...
            def __del__(self):
                ...
                #    obtain a reference to `func_`, which is currently
                #    undergoing `tp_clear`.
                global func_
                func_ = self.ref()
        ...
        class Cyclic(tuple):
            ...
            #    this case this is only us as `func_` doesn't have a
            #    finalizer.
            def __del__(self):

                # 5. Create a weakref to `func_` now. If we had created
            ...
            # 7. Now `func_` is `tp_clear`-ed. This drops the last
        ...
        def func_():
            pass
        cyc = tuple.__new__(Cyclic, (func_, latefin))

        # 1. Create a reference cycle of `cyc` and `func_`.
        func_.__module__ = cyc
        ...
        #    which will find `cyc` and `func_` as garbage.
        gc.collect()
        ...
        self.assertRegex(stdout, rb"""\A\s*func_=<function  at \S+>\s*\Z""")
