...
class TestBisect:
    ...
    def test_precomputed(self):
        for func_, data, elem, expected in self.precomputedCases:
            self.assertEqual(func_(data, elem), expected)
            self.assertEqual(func_(UserList(data), elem), expected)
    ...
    def test_optionalSlicing(self):
        for func_, data, elem, expected in self.precomputedCases:
            for lo in range(4):
                ...
                for hi in range(3,8):
                    hi = min(len(data), hi)
                    ip = func_(data, elem, lo, hi)
                    self.assertTrue(lo <= ip <= hi)
                    if func_ is self.module.bisect_left and ip < hi:
                        self.assertTrue(elem <= data[ip])
                    if func_ is self.module.bisect_left and ip > lo:
                        self.assertTrue(data[ip-1] < elem)
                    if func_ is self.module.bisect_right and ip < hi:
                        self.assertTrue(elem < data[ip])
                    if func_ is self.module.bisect_right and ip > lo:
                        self.assertTrue(data[ip-1] <= elem)
